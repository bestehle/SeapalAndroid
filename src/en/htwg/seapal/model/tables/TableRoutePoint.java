...
public class TableRoutePoint extends ATable implements IObservable {
	...
	public List<RoutePoint> selectRoute(int ID) {
		Cursor results = this.db.rawQuery("SELECT * FROM " + this.Table_Name
				+ " WHERE routeID = " + ID + " ORDER BY _id ASC", null);
	...
	public Boolean addRoutePoint(RoutePoint rp) {
		...
		if (-1 == db.insert(this.Table_Name, null, values))
			return false;
		else
			notifyObservers();
		return true;
	...
	public RoutePoint get(int ID) {
		Cursor result = db.query(this.Table_Name, null, "_id = " + ID, null,
				null, null, null, "1");
		result.moveToNext();
		RoutePoint out;
		if (result.getCount() > 0)
			out = new RoutePoint(result);
		else
			out = new RoutePoint();
		return out;
	}
	...
	public Cursor getRoute(int ID) {
		return db.query(this.Table_Name, null, "routeID = " + ID, null, null,
				null, null, "");
	}
